...
def process_ai_interaction(user_input):
    ...
    # 1.5 RAG 检索：从知识库中找相关心理学内容
    knowledge_context = search_knowledge(user_input, k=2)

    # 2. 构建动态 prompt（包含当前评分状态 + 知识上下文）
    dynamic_prompt = build_dynamic_prompt(st.session_state.scores, knowledge_context)
