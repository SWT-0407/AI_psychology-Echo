...
def build_dynamic_prompt(current_scores, rag_context=""):
    ...
    Args:
        ...
        rag_context: str, 从知识库检索到的相关书籍内容（可选）

    Returns:
        str: 完整的 system prompt
    """
    # 如果有 RAG 知识上下文，加入到 system prompt 中
    knowledge_section = ""
    if rag_context:
        knowledge_section = f"""
【参考知识库】
以下是从心理学书籍中检索到的相关内容，请在交流中自然融入这些知识（如引用书中观点、案例或方法）来增强回应的深度：
{rag_context}
"""
...
6. 如果有「参考知识库」内容，请巧妙地将其中的心理学知识融入你的回应中，让回复更有深度和专业性。
...
def build_report_prompt(current_score, current_vals, dim_names, ai_direction, rag_context=""):
    """
    构建深度解析报告的 Prompt
